...
import { useUserStore, DEFAULT_USER_INFO } from '@/store/modules/user'
...
async function fetchUserInfo(): Promise<void> {
  ...
  // 前端模式：无需登录，直接使用默认用户信息，不请求后端接口
  const { isFrontendMode } = useAppMode()
  if (isFrontendMode.value) {
    if (!userStore.info?.roles?.length) {
      userStore.setUserInfo({ ...DEFAULT_USER_INFO })
    }
