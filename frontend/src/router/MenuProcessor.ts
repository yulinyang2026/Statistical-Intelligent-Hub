...
export class MenuProcessor {
  ...
  private async processFrontendMenu(): Promise<AppRouteRecord[]> {
    ...
    // 2026-09-11 用户需求：按「菜单权限」过滤左侧功能树（如普通用户看不到「系统配置」分组）。
    //
    // 注意：本工程为 VITE_ACCESS_MODE=frontend，路由守卫在该模式下**不请求后端用户信息**（只塞默认信息），
    // 因此构建菜单时 info.menus 往往还没有值，过滤会被跳过（守卫在菜单构建后才由页面补拉真实信息）。
    // 这里在构建菜单前补拉一次真实 user/info —— 仅当「有 token 且还没有 menus」时才请求，
    // 无后端/mock 场景（无 token）保持原行为，不会把菜单整体隐藏。
    let menus = userStore.info?.menus
    if (!Array.isArray(menus) && userStore.accessToken) {
      try {
        userStore.setUserInfo(await fetchGetUserInfo())
        menus = userStore.info?.menus
      } catch {
        // 拉取失败时不做菜单权限过滤，保持菜单可用
      }
    }
    if (Array.isArray(menus)) {
      menuList = this.filterMenuByPerm(menuList, menus)
    }
  ...
  /**
   * 根据「菜单权限」过滤菜单（2026-09-11）
   *
   * 路由 meta.perm 未配置 = 不做权限控制（始终可见）；配置了则要求用户权限码集合包含它。
   * 子菜单过滤后若父节点已无子节点，由 filterEmptyMenus 统一移除空分组。
   */
  private filterMenuByPerm(menu: AppRouteRecord[], perms: string[]): AppRouteRecord[] {
    return menu.reduce((acc: AppRouteRecord[], item) => {
      const need = item.meta?.perm
      if (!need || perms.includes(need)) {
        const filteredItem = { ...item }
        if (filteredItem.children?.length) {
          filteredItem.children = this.filterMenuByPerm(filteredItem.children, perms)
        }
