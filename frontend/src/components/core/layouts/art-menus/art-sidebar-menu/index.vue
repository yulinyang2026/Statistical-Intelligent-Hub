...
<script setup lang="ts">
  ...
  // 组件内部状态
  // 2026-09-11 用户需求：左侧功能树的分组默认展开（当前仅「系统配置」一个分组），只有用户点击才收起。
  // 注意：ElMenu 的 default-openeds 只在初始化时读取一次（组件内 openedMenus 不监听该 prop），
  // 故必须首帧就给值；菜单数据由路由守卫在进入页面前写入 store，此时已可取到。
  /** ElMenu 实例引用：用于在「折叠恢复 / 尺寸切换」后主动重新展开默认分组 */
  const menuRef = useTemplateRef<{ open: (index: string) => void }>('menuRef')
  const defaultOpenedMenus = ref<string[]>(
    useMenuStore()
      .menuList.filter((menu) => !menu.meta?.isHide && (menu.children?.length ?? 0) > 0)
      .map((menu) => menu.path || String(menu.meta?.title ?? ''))
      .filter(Boolean)
  )
  ...
  // ElMenu 的 default-openeds **只在初始化时读取一次**，且**折叠状态下会被整体忽略**
  // （Element Plus 内部：openedMenus 初始值 = defaultOpeneds && !collapse ? defaultOpeneds : []）。
  // 因此侧边栏从折叠恢复、或从移动端尺寸切回桌面时，需要把默认分组主动重新展开一次，
  // 否则「分组默认展开」在「先折叠后展开」的路径下会失效（2026-09-11 加固）。
  // 延后到侧边栏展开动画之后（ElMenu 自身也会响应 collapse 变化，早于它会互相覆盖）
  const { start: startReapplyDefaultOpened } = useTimeoutFn(() => {
  ...
  watch([menuOpen, isMobileScreen], () => {
    if (!menuOpen.value || isMobileScreen.value || !defaultOpenedMenus.value.length) {
      return
    }
    startReapplyDefaultOpened()
  })
  ...
  /**
   * 拉取已保存的菜单顺序（2026-09-10 用户需求：功能树拖动排序，全局共享）
   * 侧边栏挂载后拉取一次，命中后按自定义顺序重排菜单树
   */
  onMounted(() => {
    useMenuStore().loadMenuOrder()
  })
